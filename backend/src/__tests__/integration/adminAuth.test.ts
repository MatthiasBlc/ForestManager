...
describe("Admin Auth API", () => {
  ...
  describe("POST /api/admin/auth/login", () => {
    it("should return TOTP required for admin with TOTP already configured", async () => {
      ...
      // Verifier que le cookie forestmanager_admin_session est défini
      const cookie = extractSessionCookie(res, "forestmanager_admin_session");
  ...
  describe("POST /api/admin/auth/totp/verify", () => {
    ...
    beforeEach(async () => {
      ...
      sessionCookie = extractSessionCookie(loginRes, "forestmanager_admin_session");
    });
    ...
    it("should return 401 without session (not authenticated)", async () => {
      ...
      const res = await request(app).post("/api/admin/auth/totp/verify").send({
        code: validCode,
      });
  ...
  describe("POST /api/admin/auth/logout", () => {
    it("should logout and destroy admin session", async () => {
      ...
      const meRes = await request(app).get("/api/admin/auth/me").set("Cookie", sessionCookie!);
