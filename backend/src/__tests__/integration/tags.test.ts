...
describe("Tags API", () => {
  ...
  describe("GET /api/tags", () => {
    it("should return tags with recipe counts for authenticated user", async () => {
      ...
      const res = await request(app).get("/api/tags").set("Cookie", userCookie);
    ...
    it("should filter tags by search query", async () => {
      ...
      const res = await request(app).get("/api/tags?search=ch").set("Cookie", userCookie);

      expect(res.status).toBe(200);
      expect(
        res.body.data.every((t: { name: string }) => t.name.toLowerCase().includes("ch"))
      ).toBe(true);
    });
    ...
    it("should respect limit parameter", async () => {
      ...
      const res = await request(app).get("/api/tags?limit=5").set("Cookie", userCookie);
    ...
    it("should return only GLOBAL APPROVED tags without communityId", async () => {
      ...
      const communityTag = res.body.data.find(
        (t: { name: string }) => t.name === "community_only_tag"
      );
