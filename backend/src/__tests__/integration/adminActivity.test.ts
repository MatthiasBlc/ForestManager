...
import app from "../../app";
import { createTestAdmin, loginAsAdmin } from "../setup/testHelpers";
...
describe("Admin Activity API", () => {
  ...
  describe("GET /api/admin/activity", () => {
    it("should return activity logs with pagination", async () => {
      ...
      const res = await request(app).get("/api/admin/activity").set("Cookie", adminCookie);
    ...
    it("should filter activity by type", async () => {
      ...
      expect(res.body.activities.every((a: { type: string }) => a.type === "TAG_CREATED")).toBe(
        true
      );
    });
