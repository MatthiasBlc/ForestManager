...
describe("Admin Units API", () => {
  ...
  describe("GET /api/admin/units", () => {
    it("should return all units with usage counts", async () => {
      ...
      const res = await request(app).get("/api/admin/units").set("Cookie", adminCookie);
    ...
    it("should filter by search query", async () => {
      ...
      expect(res.status).toBe(200);
      expect(res.body.units.some((u: { name: string }) => u.name === "millilitre_search")).toBe(
        true
      );
  ...
  describe("POST /api/admin/units", () => {
    it("should create a new unit", async () => {
      const res = await request(app).post("/api/admin/units").set("Cookie", adminCookie).send({
        name: "New Unit",
        abbreviation: "nu",
        category: "COUNT",
        sortOrder: 10,
  ...
  describe("DELETE /api/admin/units/:id", () => {
    ...
    it("should create audit log on delete", async () => {
      ...
      await request(app).delete(`/api/admin/units/${unit.id}`).set("Cookie", adminCookie);
    ...
    it("should prevent deleting a unit used in recipes", async () => {
      ...
      const recipe = await testPrisma.recipe.create({
        data: {
          ...
          servings: 4,
          creatorId: user.id,
          steps: { create: [{ order: 0, instruction: "Test" }] },
        },
      });
...
describe("User Units API", () => {
  ...
  describe("GET /api/units", () => {
    it("should return units grouped by category", async () => {
      ...
      const res = await request(app).get("/api/units").set("Cookie", userCookie);
      ...
      const hasVolume = res.body.data.VOLUME?.some(
        (u: { name: string }) => u.name === "user_litre"
      );
