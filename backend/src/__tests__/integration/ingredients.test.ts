...
describe("Ingredients API", () => {
  ...
  describe("GET /api/ingredients", () => {
    it("should return ingredients with recipe counts for authenticated user", async () => {
      ...
      const res = await request(app).get("/api/ingredients").set("Cookie", userCookie);
    ...
    it("should filter ingredients by search query", async () => {
      await createTestRecipe(userId, {
        ingredients: [{ name: "chocolate" }, { name: "cheese" }, { name: "vanilla" }],
      });

      const res = await request(app).get("/api/ingredients?search=ch").set("Cookie", userCookie);

      expect(res.status).toBe(200);
      expect(
        res.body.data.every((i: { name: string }) => i.name.toLowerCase().includes("ch"))
      ).toBe(true);
    });
    ...
    it("should respect limit parameter", async () => {
      ...
      const res = await request(app).get("/api/ingredients?limit=5").set("Cookie", userCookie);
    ...
    it("should only count recipes owned by the authenticated user", async () => {
      ...
      expect(res.status).toBe(200);
      const sharedIngredient = res.body.data.find(
        (i: { name: string }) => i.name === "shared_ingredient"
      );
