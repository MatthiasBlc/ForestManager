...
describe("Admin Tags API", () => {
  ...
  describe("GET /api/admin/tags", () => {
    it("should return all tags with recipe counts", async () => {
      ...
      const res = await request(app).get("/api/admin/tags").set("Cookie", adminCookie);
    ...
    it("should filter tags by search query", async () => {
      ...
      const res = await request(app).get("/api/admin/tags?search=ch").set("Cookie", adminCookie);

      expect(res.status).toBe(200);
      expect(
        res.body.tags.every((t: { name: string }) => t.name.toLowerCase().includes("ch"))
      ).toBe(true);
    });
    ...
    it("should filter by scope=GLOBAL", async () => {
      ...
      const res = await request(app).get("/api/admin/tags?scope=GLOBAL").set("Cookie", adminCookie);
  ...
  describe("POST /api/admin/tags", () => {
    ...
    it("should return 400 when name is missing", async () => {
      const res = await request(app).post("/api/admin/tags").set("Cookie", adminCookie).send({});
  ...
  describe("DELETE /api/admin/tags/:id", () => {
    it("should delete a tag", async () => {
      ...
      const res = await request(app).delete(`/api/admin/tags/${tag.id}`).set("Cookie", adminCookie);
