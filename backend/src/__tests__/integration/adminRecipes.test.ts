...
describe('Admin Recipes API', () => {
  ...
  describe('GET /api/admin/tags/:id/recipes', () => {
    ...
    it('should filter out deleted recipes by default', async () => {
      ...
      const recipe = await createTestRecipe(user.id, { title: 'Active Recipe', tags: ['tagfilter'] });
      const deletedRecipe = await createTestRecipe(user.id, { title: 'Deleted Recipe', tags: ['tagfilter'] });
    ...
    it('should include deleted recipes when includeDeleted=true', async () => {
      ...
      const deletedRecipe = await createTestRecipe(user.id, { title: 'Deleted2', tags: ['tagdeleted'] });
