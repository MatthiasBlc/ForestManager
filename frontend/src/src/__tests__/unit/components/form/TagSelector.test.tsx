...
describe("TagSelector", () => {
  ...
  it("should render with placeholder when empty", () => {
    render(<TagSelector value={[]} onChange={mockOnChange} placeholder="Search tags..." />);

    expect(screen.getByPlaceholderText("Search tags...")).toBeInTheDocument();
  });

  it("should display selected tags", () => {
    render(<TagSelector value={["dessert", "chocolate"]} onChange={mockOnChange} />);
  ...
  it("should hide placeholder when tags are selected", () => {
    render(
      <TagSelector value={["dessert"]} onChange={mockOnChange} placeholder="Search tags..." />
    );
  ...
  it("should add tag on Enter when allowCreate is true", async () => {
    const user = userEvent.setup();
    render(<TagSelector value={[]} onChange={mockOnChange} allowCreate={true} />);
  ...
  it("should accept communityId prop without error", () => {
    render(<TagSelector value={[]} onChange={mockOnChange} communityId="community-123" />);
